defmodule TimeMangerBackend.ClocksTest do
  ...
  describe "clocks" do
    ...
    import TimeMangerBackend.ClocksFixtures
    ...
    test "create_clock/1 with valid data creates a clock" do
      valid_attrs = %{status: true, time: ~N[2025-09-25 08:12:00]}

      assert {:ok, %Clock{} = clock} = Clocks.create_clock(valid_attrs)
      assert clock.status == true
      assert clock.time == ~N[2025-09-25 08:12:00]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~N[2025-09-26 08:12:00]}

      assert {:ok, %Clock{} = clock} = Clocks.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~N[2025-09-26 08:12:00]
    end
